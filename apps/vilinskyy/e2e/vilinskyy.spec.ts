...
test.describe('Vilinskyy Portfolio', () => {
  ...
  test.describe('Navigation', () => {
    test('should navigate between pages', async ({ page }) => {
      // Check home page
      await expect(page.locator('h1')).toContainText('Alexander Vilinskyy')
    ...
    test('should highlight active navigation item', async ({ page }) => {
      ...
      await expect(page.locator(`nav a[href="/"]`)).toHaveClass(new RegExp(activeClass))
      
      // Navigate to Visuals and check it's active
      await page.click('nav a:has-text("Visuals")')
      await expect(page.locator(`nav a[href="/visuals"]`)).toHaveClass(new RegExp(activeClass))
      await expect(page.locator(`nav a[href="/"]`)).not.toHaveClass(new RegExp(activeClass))
    })
  })
  ...
  test.describe('Hero Section', () => {
    test('should display hero content', async ({ page }) => {
      await expect(page.locator('h1')).toContainText('Alexander Vilinskyy')
      await expect(page.locator('text=Digital designer and founder')).toBeVisible()
      await expect(page.locator('text=Super Clear')).toBeVisible()
      
      // Check social links
      const socialLinks = page.locator('a[aria-label]')
      await expect(socialLinks).toHaveCount(5) // Twitter, LinkedIn, Instagram, YouTube, Bluesky
    })
  ...
  test.describe('Project Listings', () => {
    ...
    test('should expand project details on click', async ({ page }) => {
      ...
      await expect(page.locator('button:has-text("Read More")').first()).toBeVisible()
    })

    test('should have proper ARIA attributes for expandable sections', async ({ page }) => {
      ...
      await expect(page.locator('button:has-text("Read Less")').first()).toHaveAttribute('aria-expanded', 'true')
    })
  })
  ...
  test.describe('Newsletter Form', () => {
    test('should validate email input', async ({ page }) => {
      const newsletterSection = page.locator('h2:has-text("Stay in the Loop")').locator('..')
      const emailInput = newsletterSection.locator('input[type="email"]')
      const submitButton = newsletterSection.locator('button:has-text("Subscribe")')
  ...
  test.describe('Multi-column Layouts', () => {
    test('should display productivity stack', async ({ page }) => {
      await expect(page.locator('h2:has-text("My Productivity Stack")')).toBeVisible()
    ...
    test('should display achievements', async ({ page }) => {
      await expect(page.locator('h2:has-text("Notable Achievements")')).toBeVisible()
      await expect(page.locator('h3:has-text("Design Leadership")')).toBeVisible()
      await expect(page.locator('h3:has-text("Entrepreneurship")')).toBeVisible()
  ...
  test.describe('Visuals Page', () => {
    test('should display gallery', async ({ page }) => {
      ...
      const filterButtons = page.locator('button:has-text("All"), button:has-text("Product Design")')
    ...
    test('should filter gallery by category', async ({ page }) => {
      ...
      for (let i = 0; i < count; i++) {
        await expect(galleryItems.nth(i).locator('.itemCategory')).toContainText('Product Design')
      }
    })
  ...
  test.describe('Networking Course Page', () => {
    test('should display course information', async ({ page }) => {
      ...
      // Check sections
      await expect(page.locator('h2:has-text("What You\'ll Learn")')).toBeVisible()
      await expect(page.locator('h2:has-text("Course Curriculum")')).toBeVisible()
  ...
  test.describe('Loom Roast Page', () => {
    ...
    test('should validate submission form', async ({ page }) => {
      ...
      // Check success
      await expect(page.locator('text=Your project has been submitted')).toBeVisible()
  ...
  test.describe('Accessibility', () => {
    ...
    test('should support keyboard navigation for expandable sections', async ({ page }) => {
