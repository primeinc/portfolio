...
test.describe('Measure Hero Section Spacing', () => {
  test('should measure hero section and logo grid spacing', async ({
    page,
  }) => {
    ...
    await page.screenshot({
      path: '../screenshot-hero-measurement.png',
      fullPage: false,
    })
    ...
    const measurements = await page.evaluate(() => {
      ...
      const subtitle =
        mainHeading?.nextElementSibling?.tagName === 'P'
      ...
      if (logoMeasurements.length >= 2) {
        ...
        horizontalGap =
          logoMeasurements[1].left -
          (logoMeasurements[0].left + logoMeasurements[0].width)
      }
      if (logoMeasurements.length >= 4) {
        // Find first logo in second row (assuming grid layout)
        const firstRowY = logoMeasurements[0].top
        const secondRowLogo = logoMeasurements.find(
          (logo) => logo.top > firstRowY + 10
        )
        if (secondRowLogo) {
          const firstRowLogo = logoMeasurements[0]
          verticalGap =
            secondRowLogo.top - (firstRowLogo.top + firstRowLogo.height)
        }
      }
      ...
      return {
        ...
        spacing: {
          ...
          measurement: heroToLogoSpacing
            ? `${Math.round(heroToLogoSpacing)}px`
            : 'not found',
        },
        viewport: {
    ...
    console.log(
      `Hero to Logo Grid Spacing: ${measurements.spacing.measurement}`
    )
    console.log(`Logo Grid Found: ${measurements.logoGrid.found}`)
    console.log(`Number of Logos: ${measurements.logoGrid.logoCount}`)
    console.log(
      `Horizontal Gap: ${measurements.logoGrid.calculatedGaps.horizontal}px`
    )
    console.log(
      `Vertical Gap: ${measurements.logoGrid.calculatedGaps.vertical}px`
    )
    ...
    if (measurements.logoGrid.styles) {
      ...
      console.log(
        `- Grid Template Columns: ${measurements.logoGrid.styles.gridTemplateColumns}`
      )
    }
  })
})
