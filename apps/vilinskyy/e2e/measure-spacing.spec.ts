...
test.describe('Measure Real Site Spacing', () => {
  test('should measure exact spacing and dimensions', async ({ page }) => {
      ...
      const sectionSpacings = sections.map((section, index) => {
        ...
        return {
          ...
          spacingToNext: nextSection
            ? nextSection.getBoundingClientRect().top - rect.bottom
            : null,
        }
      })
    ...
    console.log(
      `- Hero to logo grid spacing: ${measurements.hero.spacingToLogoGrid}px`
    )
    console.log(
      `- Logo grid gap: ${measurements.logoGrid.container?.gap || 'not found'}`
    )
    console.log(
      `- Logo grid dimensions: ${measurements.logoGrid.container?.width}x${measurements.logoGrid.container?.height}`
    )
  })

  test('should measure specific grid and layout properties', async ({
    page,
  }) => {
    await page.goto('https://vilinskyy.com', { waitUntil: 'networkidle' })
    await page.waitForTimeout(2000)
    ...
    const detailedMeasurements = await page.evaluate(() => {
      ...
      const sectionStyles = parentSection
        ? window.getComputedStyle(parentSection)
        : null
      ...
      const measurements = {
        ...
        calculatedGaps:
          logos.length > 1
            ? {
                horizontal: logos[1]
                  ? logos[1].getBoundingClientRect().left -
                    logos[0].getBoundingClientRect().right
                  : 0,
                vertical:
                  logos[3] && logos[0]
                    ? logos[3].getBoundingClientRect().top -
                      logos[0].getBoundingClientRect().bottom
                    : 0,
              }
            : null,
      }
    ...
    writeFileSync(
      '../spacing-measurements-detailed.json',
