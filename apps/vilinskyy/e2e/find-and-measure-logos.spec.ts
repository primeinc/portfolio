...
test.describe('Find and Measure Logo Grid', () => {
  test('should find logo grid and measure spacing', async ({ page }) => {
    ...
    await page.screenshot({ 
      path: '/home/will/local_dev/portfolio/apps/vilinskyy/screenshot-full-page-logos.png',
      fullPage: true 
    })
    ...
    const pageStructure = await page.evaluate(() => {
      ...
      document.querySelectorAll('section').forEach((section, index) => {
        ...
        const hasLogos = text.toLowerCase().includes('companies') || 
    ...
    pageStructure.forEach(section => {
      console.log(`Section ${section.index}: ${section.hasLogos ? '✓ LOGOS' : '✗'} - ${section.imageCount} images - Top: ${section.top}px`)
      console.log(`  Text preview: "${section.text.slice(0, 80)}..."`)
      console.log(`  Class: ${section.className}`)
    })
    
    // Find the most likely logo section
    const logoSection = pageStructure.find(s => s.hasLogos) || pageStructure.find(s => s.imageCount >= 6)
    
    if (logoSection) {
      // Scroll to the logo section
      await page.evaluate((top) => window.scrollTo(0, top - 100), logoSection.top)
      ...
      const measurements = await page.evaluate((sectionIndex) => {
        ...
        if (!logoContainer || logoContainer.querySelectorAll('img').length < 4) {
        ...
        return {
          ...
          gaps: {
            ...
            avgHorizontal: gaps.horizontal.length > 0 ? 
              Math.round(gaps.horizontal.reduce((a, b) => a + b, 0) / gaps.horizontal.length) : 0,
            avgVertical: gaps.vertical.length > 0 ? 
              Math.round(gaps.vertical.reduce((a, b) => a + b, 0) / gaps.vertical.length) : 0
          }
        }
      }, logoSection.index)
      ...
      if (!measurements.error) {
        ...
        console.log(`Grid Template: ${measurements.container.gridTemplateColumns}`)
        console.log(`\nLayout:`)
        console.log(`- Rows: ${measurements.layout.rows}`)
        console.log(`- Images per row: ${measurements.layout.imagesPerRow.join(', ')}`)
        console.log(`- Total images: ${measurements.layout.totalImages}`)
        console.log(`\nCalculated Gaps:`)
        console.log(`- Horizontal: ${measurements.gaps.avgHorizontal}px (samples: ${measurements.gaps.horizontal.length})`)
        console.log(`- Vertical: ${measurements.gaps.avgVertical}px (samples: ${measurements.gaps.vertical.length})`)
      } else {
