...
test.describe('Measure Visible Logo Grid', () => {
  test('should measure the visible logo grid', async ({ page }) => {
    ...
    const measurements = await page.evaluate(() => {
      ...
      const potentialLogos = allImages.filter((img) => {
        ...
        return (
          ...
          (src.includes('grammarly') ||
            src.includes('spark') ||
            src.includes('pervasive') ||
            src.includes('freshbrief') ||
            src.includes('wikiflow') ||
            src.includes('axis') ||
            src.includes('tickets') ||
            src.includes('readme') ||
            src.includes('context') ||
            src.includes('daylight') ||
            src.includes('tezos') ||
            src.includes('complex') ||
            src.includes('decipad') ||
            src.includes('jammable') ||
            src.includes('somete') ||
      ...
      while (
        logoContainer &&
        !Array.from(logoContainer.children).every(
      ...
      const containerStyles = logoContainer
        ? window.getComputedStyle(logoContainer)
        : null

      // Measure each logo
      const logoMeasurements = potentialLogos.map((img, index) => {
        const rect = img.getBoundingClientRect()
        // Style information available if needed
        return {
          index,
          width: rect.width,
          height: rect.height,
      ...
      if (logoContainer) {
        ...
        if (
          prevSibling &&
          (prevSibling.querySelector('h1, h2, p') ||
      ...
      const spacingFromHero =
        heroText && containerRect ? containerRect.top - heroText.bottom : null
    ...
    const finalMeasurements = {
      ...
      summary: {
        ...
        gridColumns:
          measurements.container?.gridTemplateColumns || 'repeat(5, 1fr)',
    ...
    console.log(`\nSUMMARY:`)
    console.log(
      `- Hero to Logo Grid Spacing: ${finalMeasurements.summary.heroToLogoSpacing}px`
    )
    console.log(
      `- Logo Grid Gap (Horizontal): ${finalMeasurements.summary.logoGridGap.horizontal}px`
    )
    console.log(
      `- Logo Grid Gap (Vertical): ${finalMeasurements.summary.logoGridGap.vertical}px`
    )
    ...
    console.log(
      `- Container Padding: ${finalMeasurements.summary.containerPadding}`
    )
    console.log(
      `- Layout: ${finalMeasurements.summary.logoLayout.rows} rows × ${finalMeasurements.summary.logoLayout.columns} columns`
    )
    ...
    if (measurements.logos?.count) {
      ...
      console.log(
        `- Logos per row: ${measurements.logos.layout.perRow.join(', ')}`
      )
    }
  })
})
